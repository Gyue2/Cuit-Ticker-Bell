...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  return (
    <div className="flex flex-col p-3.5 md:p-4 mb-6 rounded-xl bg-white dark:bg-[#1A202C] border border-slate-200 dark:border-[#2D3748] shadow-md transition-colors">
      
      {/* Mobile Toggle Button */}
      <button 
        onClick={() => setIsExpanded(!isExpanded)}
        className="md:hidden w-full flex items-center justify-center gap-2 px-3 py-2 bg-slate-50 hover:bg-slate-100 dark:bg-[#171923] dark:hover:bg-slate-800 text-slate-700 dark:text-gray-300 rounded-lg text-xs font-bold border border-slate-200 dark:border-[#2D3748] transition-colors cursor-pointer"
      >
        <ListFilter className="w-4 h-4" />
        <span>{isExpanded ? "필터 닫기 ▲" : "검색 및 필터 옵션 열기 ▼"}</span>
      </button>

      <div className={`${isExpanded ? "flex" : "hidden"} md:flex flex-col lg:flex-row lg:items-center justify-between gap-3 w-full mt-3 md:mt-0`}>
